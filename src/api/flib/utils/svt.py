...
class AudioProcessor(object):
    ...
    def spectral_centroid(self, seek_point, spec_range=120.0):
        ...
        samples = self.read(
            seek_point - self.fft_size / 2, self.fft_size, True)
        ...
        if energy > 1e-20:
            ...
            spectral_centroid = (
                spectrum * self.spectrum_range).sum() / (energy * (length - 1)) * self.samplerate * 0.5

            # clip > log10 > scale between 0 and 1
            spectral_centroid = (math.log10(self.clip(
                spectral_centroid, self.lower, self.higher)) - self.lower_log) / (self.higher_log - self.lower_log)
...
def interpolate_colors(colors, flat=False, num_colors=256):
    ...
    for i in range(num_colors):
        ...
        if alpha > 0:
            r = (1.0 - alpha) * \
                colors[index_int][0] + alpha * colors[index_int + 1][0]
            g = (1.0 - alpha) * \
                colors[index_int][1] + alpha * colors[index_int + 1][1]
            b = (1.0 - alpha) * \
                colors[index_int][2] + alpha * colors[index_int + 1][2]
        else:
...
class WaveformImage(object):
    ...
    def save(self, filename):
        ...
        for x in range(self.image_width):
            self.pix[x, self.image_height /
                     2] = tuple(map(lambda p: p + a, self.pix[x, self.image_height / 2]))
    ...
    def __init__(self, image_width, image_height, fft_size, f_max, f_min, nyquist_freq, palette, logspec):
        ...
        for y in range(self.image_height):
            ...
            if logspec == 1:
                # log scale
                freq = math.pow(
                    10.0, y_min + y / (image_height - 1.0) * (y_max - y_min))
            else:
                # arithmetic scale
                freq = f_min + y / (image_height - 1.0) * (f_max - f_min)
            # uses the nyquist frequency to allow files of different sampling
            # rate
        ...
        # so we store all the pixels in an array and then create the image when
        # saving
        self.pixels = []
...
#    processor = AudioProcessor(audio_file, fft_size, channel, numpy.hanning)
    ...
    spectrogram = SpectrogramImage(
        image_width, image_height, fft_size, f_max, f_min, nyquist_freq, palette, logspec)
...
if __name__ == '__main__':
    parser = optparse.OptionParser(
        "usage: %prog [options] input-filename", conflict_handler="resolve")
    parser.add_option("-a", "--waveout", action="store", dest="output_filename_w",
                      type="string", help="output waveform image (default input filename + _w.png)")
    parser.add_option("-o", "--wavefile", action="store", dest="wavefile",
                      type="int", help="draw waveform image (yes:1, no: 0; default: no)")
    parser.add_option("-s", "--specout", action="store", dest="output_filename_s",
                      type="string", help="output spectrogram image (default input filename + _s.png)")
    parser.add_option("-w", "--width", action="store", dest="image_width",
                      type="int", help="image width in pixels (default %default)")
    parser.add_option("-h", "--height", action="store", dest="image_height",
                      type="int", help="image height in pixels (default %default)")
    ...
    parser.add_option("-n", "--window", action="store", dest="window", type="string",
                      help="window to use for the FFT: bartlett, blackman, hanning, hamming, or kaiser (default %default)")
    parser.add_option("-m", "--fmax", action="store", dest="f_max",
                      type="int", help="Maximum freq to draw, in Hz (default %default)")
    parser.add_option("-i", "--fmin", action="store", dest="f_min",
                      type="int", help="Minimum freq to draw, in Hz (default %default)")
    parser.add_option("-p", "--palette", action="store", dest="palette", type="int",
                      help="Which color palette to use to draw the spectrogram, 1 for black background and 2 for white background (default %default)")
    parser.add_option("-c", "--channel", action="store", dest="channel", type="int",
                      help="Which channel to draw in a stereo file, 1 for left or 2 for right (default %default)")
    parser.add_option("-l", "--log", action="store_true", dest="logspec",
                      help="Draw the spectrogram in log scale, 0 for no (arithmetic) or 1 for log scale")
    parser.add_option("-v", "--version", action="store_true",
                      dest="version", help="Display version information")

    parser.set_defaults(output_filename_w=None, output_filename_s=None, image_width=500, image_height=170,
                        fft_size=2048, f_max=22050, f_min=10, wavefile=0, palette=1, channel=1, window="hanning", logspec=0)
    ...
    if not options.version:
        ...
        if len(args) > 1 and (options.output_filename_w != None or options.output_filename_s != None):
            parser.error(
                "when processing multiple files you can't define the output filename!")

        # process all files so the user can use wildcards like *.wav
        for input_file in args:
            if options.channel == 2:
                output_file_w = options.output_filename_w or input_file + \
                    "_w_r.png"
                output_file_s = options.output_filename_s or input_file + \
                    "_s_r.png"
            else:
                output_file_w = options.output_filename_w or input_file + \
                    "_w.png"
            ...
            args = (input_file, output_file_w, output_file_s, options.image_width, options.image_height, options.fft_size,
                    options.f_max, options.f_min, options.wavefile, options.palette, options.channel, options.window, options.logspec)
